    ...
    case ARM_CP_DC_GVA:
        {
            ...
            if (s->ata[0]) {
                ...
                gen_helper_stzgm_tags(tcg_env, clean_addr, tag, tcg_constant_i32(s->mtx));
            }
        }
        return;
    case ARM_CP_DC_GZVA:
...
static bool trans_STGP(DisasContext *s, arg_ldstpair *a)
{
    ...
    if (s->ata[0]) {
        if (tb_cflags(s->base.tb) & CF_PARALLEL) {
            gen_helper_stg_parallel(tcg_env, dirty_addr, dirty_addr, tcg_constant_i32(s->mtx));
        } else {
            gen_helper_stg(tcg_env, dirty_addr, dirty_addr, tcg_constant_i32(s->mtx));
        }
    }
...
static bool do_STG(DisasContext *s, arg_ldst_tag *a, bool is_zero, bool is_pair)
{
    ...
    if (!s->ata[0]) {
        ...
        if (is_pair) {
            gen_helper_st2g_parallel(tcg_env, addr, tcg_rt, tcg_constant_i32(s->mtx));
        } else {
            gen_helper_stg_parallel(tcg_env, addr, tcg_rt, tcg_constant_i32(s->mtx));
        }
    } else {
        if (is_pair) {
